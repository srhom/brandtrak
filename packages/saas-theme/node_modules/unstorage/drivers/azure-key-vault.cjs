...
var _default = (0, _utils.defineDriver)(opts => {
  ...
  const getKeyVaultClient = () => {
    ...
    if (!vaultName) {
      throw new Error("[unstorage] [key-vault] Azure Key Vault driver requires a vault name to be provided.");
    }
    if (pageSize > 25) {
      throw new Error("[unstorage] [key-vault] pageSize cannot be greater than 25.");
    }
